...
class Mutated : public Device
{
  public:
    Mutated(Mutate& mut, Device& dev);
...
class MutatedOwned : public Mutated
{
  public:
    MutatedOwned(std::unique_ptr<Mutate>&& mut, std::unique_ptr<Device>&& dev);

  private:
    std::unique_ptr<Mutate> mut;
