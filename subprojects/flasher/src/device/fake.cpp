...
stdplus::span<std::byte> Fake::readAt(stdplus::span<std::byte> buf, size_t offset)
{
    return file->readAt(buf, offset);
}

stdplus::span<const std::byte> Fake::writeAt(stdplus::span<const std::byte> data, size_t offset)
{
    switch (info.type) {
	    case Type::Nor: {
...
void Fake::eraseBlocks(size_t idx, size_t num)
{
    ...
    {
        file->writeAtExact(
            stdplus::raw::asSpan<std::byte>(erase_contents), i * erase_contents.size());
    }
}

Device::DeviceInfo Fake::buildDeviceInfo(File& file, Type type,
                                         size_t erase)
{
    DeviceInfo ret;
    ret.type = type;
...
class FakeType : public DeviceType
{
  public:
    ...
    void printHelp() const override
    {
        ...
        fmt::print(
            stderr,
            "    type=TYPE        required  The type of flash to use (nor, simple)\n");
        fmt::print(
            stderr,
        ...
        fmt::print(stderr, "    FILE             required  Backing file specification\n");
    }
};
