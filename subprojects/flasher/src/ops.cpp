...
void automatic(Device&, size_t, File&,
               size_t, Mutate &, size_t,
               std::optional<size_t>, bool)
{
  throw std::runtime_error("Not implemented");
}

void read(Device&, size_t, File&,
          size_t, Mutate&, size_t, std::optional<size_t>)
{
  throw std::runtime_error("Not implemented");
}

void write(Device&, size_t, File&, size_t,
           Mutate&, size_t, std::optional<size_t>,
           bool)
{
  throw std::runtime_error("Not implemented");
}
           ...
           std::optional<size_t> stride_size, bool noread)
{
    ...
    {
	 throw std::invalid_argument(fmt::format("Device offset not divisible by erase, {} % {} != 0", dev_offset, erase_size));
    }
    ...
    {
	 throw std::invalid_argument(fmt::format("Stride not divisible by erase, {} % {} != 0", stride, erase_size));
    }
   if (dev_offset > dev.getSize())
   {
	  throw std::invalid_argument(fmt::format("Device smaller than offset, {} < {}", dev.getSize(), dev_offset));
   }
    max_size = std::min(max_size, dev.getSize() - dev_offset);
    if (max_size % erase_size != 0)
    {
	 throw std::invalid_argument(fmt::format("Size not divisible by erase, {} % {} != 0", max_size, erase_size));
    }
    ...
    for (size_t i = dev_offset; i < max_size + dev_offset; i += stride)
    {
	   ...
	   log(LogLevel::Info, " RD@{}#{}", i, bytes);
	      if (!dev.needsErase(buf.subspan(0, bytes), erased.subspan(0, bytes)))
	      {
		      continue;
	      }
	   }
            ...
            Mutate& mutate, size_t max_size, std::optional<size_t> stride_size)
{
   ...
   while (max_size > 0)
   {
       if (file_data.size() < stride && max_size > file_data.size() && !eof)
       {
	       try
	       {
		  auto new_file_data = file.readAt(file_buf.subspan(file_data.size(), std::min(stride, max_size - file_data.size())), file_offset);
		  log(LogLevel::Info, " VF@{}#{}", dev_offset, new_file_data.size());
		  file_data = file_buf.subspan(0, file_data.size() + new_file_data.size());
       ...
       {
	   auto new_size = std::min(stride, std::min(max_size, dev.getSize() - dev_offset) - dev_data.size());
	   if (new_size == 0)
	   {
		   throw std::runtime_error("Device not large enough for verification file");
	   }
	   auto new_dev_data = dev.readAt(dev_buf.subspan(dev_data.size(), new_size), dev_offset);
           log(LogLevel::Info, " VD@{}#{}", dev_offset, new_dev_data.size());
           mutate.reverse(new_dev_data, dev_offset);
	   dev_data = dev_buf.subspan(0, dev_data.size() + new_dev_data.size());
       ...
       {
	       log(LogLevel::Info, "\n");
	   throw std::runtime_error(fmt::format("Bytes #{} D@{} != F@{}", compared, dev_offset - dev_data.size(), file_offset - file_data.size()));
       }
       std::memmove(file_data.data(), file_data.data() + compared, file_data.size() - compared);
       file_data = file_data.subspan(0, file_data.size() - compared);
       std::memmove(dev_data.data(), dev_data.data() + compared, dev_data.size() - compared);
