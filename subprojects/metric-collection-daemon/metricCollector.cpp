...
void MetricCollector::registerDaemons()
{
    ...
    for (const auto& procEntry : std::filesystem::directory_iterator(procPath))
    {
        ...
        if (daemonObjectPath == std::nullopt)
        {
            continue;
        }
...
void MetricCollector::update()
{
    ...
    // Update Associations property.
    // (Associations vector is updated in updatePorts and updateDaemons)
    association_interface->set_property("Associations", associations);
}
