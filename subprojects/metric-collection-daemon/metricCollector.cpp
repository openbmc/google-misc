...
void MetricCollector::removeAssociation(const std::string objectPath)
{
    ...
    {
        auto association = *it;
        std::cerr << std::get<0>(association) << " " << std::get<1>(association)
                  << " " << std::get<2>(association) << std::endl;
...
void MetricCollector::registerDaemons()
{
    ...
    for (const auto& procEntry : std::filesystem::directory_iterator(procPath))
    {
        ...
        std::cerr << "registering daemon " << *pid << std::endl;
...
void MetricCollector::updateDaemons()
{
    ...
    for (const auto& procEntry : std::filesystem::directory_iterator(procPath))
    {
        ...
        std::cerr << "Added pid " << *pid << std::endl;
        allDaemonPids.emplace(*pid);
    }

    std::cerr << "got all pids" << std::endl;
    ...
    for (auto it = bmcDaemons.begin(); it != bmcDaemons.end();)
    {
        ...
        if (!allDaemonPids.contains(daemonPid))
        {
            ...
            std::cerr << "removed from map" << std::endl;
            std::cerr << "removing " << daemon.getObjectPath() << std::endl;
            removeAssociation(daemon.getObjectPath());
            continue;
        }
        std::cerr << "updating pid " << daemonPid << std::endl;
        // Update daemon info of existing daemons
        daemon.updateInfo();
        ++it;
    }

    std::cerr << "Removed all nonexistent pids" << std::endl;
    ...
    for (const auto& daemonPid : allDaemonPids)
    {
        ...
        {
            std::cerr << "adding new daemon " << daemonPid << std::endl;
            std::optional<std::string> daemonObjectPath =
...
void MetricCollector::update()
{
    ...
    updatePorts();
    std::cerr << "Updated ports" << std::endl;
    updateDaemons();
    std::cerr << "Updated daemons" << std::endl;
}
