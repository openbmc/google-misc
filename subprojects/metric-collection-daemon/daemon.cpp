...
void Daemon::updateProcessStatistics()
{
    ...
    // This means procStat was incorrectly parsed
    if (parsedProcFile.size() < 13)
    {
        return;
    }
...
void Daemon::updateFdCount()
{
    ...
    {
        std::cerr << "Failed to open file descriptors for pid " << pid
                  << std::endl;
    }
}
...
void Daemon::updateInfo()
{
    ...
    updateCommandLine();
    std::cerr << "Got daemon commandline " << commandLine << std::endl;
    updateProcessStatistics();
    std::cerr << "Got daemon procstat " << userSeconds << " " << kernelSeconds
              << " " << memoryUsage << std::endl;

    updateFdCount();
    std::cerr << "Got daemon fd " << fileDescriptors << std::endl;

    updateRestartCount();
    std::cerr << "Got daemon restart " << restartCount << std::endl;

    updateUptime();
    std::cerr << "Got daemon uptime " << uptime << std::endl;
}
