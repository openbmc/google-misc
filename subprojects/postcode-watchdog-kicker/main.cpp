...
static constexpr auto MATCH = "type='signal',"
...
int main()
{
    ...
    else
    {
         ...
         */
        fprintf(stderr,
                "Unable to open or process enable configuration: '%s'\n",
                ENABLE_CONF);
    }
...
static void ResetWatchdog(uint64_t code)
{
    ...
    if (enable)
    {
        ...
        // when we see it.
        auto pEnbMsg = WatchdogBus.new_method_call(
    ...
    auto pWriteMsg = WatchdogBus.new_method_call(
        WATCHDOG_SERVICE, WATCHDOG_PATH, PROP_INTF, "Set");
