 ...
 *  - FMAP & HMAP (go/fmap-hmap)
 *  - BBINFO (go/bios-info)
 *  - BIOS signature header (go/bios-signing)
 *
 *  See go/platforms-security/image_descriptor for companion documentation.
 *
 ...
 * See go/haven-payload-rollback. There must be zero or one image_mauv
 ...
 * structure. See go/haven-payload-controlled-lockdown. There must be zero or
