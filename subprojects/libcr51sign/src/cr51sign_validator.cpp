...
bool Cr51SignValidatorIpml::readImage(std::span<std::byte> file, size_t size)
{
    if (file.size() != size)
    {
        return false;
    ...
    firmwareFileBuf.resize(size);
    std::memcpy(firmwareFileBuf.data(), file.data(), size);
    return true;
}
    ...
    Cr51SignValidatorIpml::hashDescriptor(struct libcr51sign_ctx* ctx)
{
    ...
    ec = hash_update(ctx,
                     firmwareFileBuf.data() + ctx->descriptor.descriptor_offset,
