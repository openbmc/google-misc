...
std::optional<struct libcr51sign_validated_regions>
    validateDescriptor(struct libcr51sign_ctx* ctx, bool cleanImg)
{
    ...
    auto mask = IMAGE_REGION_STATIC;

    // On unclean image, the persistent regions are copied over and can't be
    // validated.
    if (!cleanImg)
    {
        mask |= IMAGE_REGION_PERSISTENT | IMAGE_REGION_PERSISTENT_RELOCATABLE |
                IMAGE_REGION_PERSISTENT_EXPANDABLE;
    }

    for (size_t i = 0; i < ctx->descriptor.region_count; ++i)
    {
        const auto& region = imageRegions.image_regions[i];

        // Skip regions that are static or persistent on unclean image
        if (region.region_attributes & mask)
        {
            continue;
        }

        if (region.region_size + region.region_offset >= firmwareFileBuf.size())
        {
            fmt::print(
                stderr,
                "The region offset + region size is larger than the buffer"
                "region offert: {} + size {} > ",
                region.region_name, region.region_offset, region.region_size,
                firmwareFileBuf.size());
            return std::nullopt;
        }

        // Check if the non-static regions are all zero.
        for (size_t j = 0; j < region.region_size; ++j)
        {
            if (firmwareFileBuf[j + region.region_offset])
            {
                fmt::print(stderr,
                           "The non-status region of the image is not all "
                           "zero: region {}",
                           region.region_name);
                return std::nullopt;
            }
