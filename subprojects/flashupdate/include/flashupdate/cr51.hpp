...
{
  public:
    /** @brief Constructor for Cr51 helper
     *
     ...
     */
    Cr51(std::string_view file, uint32_t size, Config::Key keys) :
        file(file), size(size), keys(keys),
     ...
     */
    std::vector<struct image_region> persistentRegions();
