...
std::string FlashHelper::readMtdFile(const std::string& filename)
{
...
std::optional<std::pair<std::string, uint32_t>> Flash::getFlash(bool primary)
{
    ...
    auto argFile =
        flasher::ModArgs(fmt::format("{}/bind", config.flash.driver));
    auto file = openFile(argFile, OpenFlags(OpenAccess::WriteOnly)
                                      .set(OpenFlag::Create)
                                      .set(OpenFlag::Trunc));
    ...
    file->writeAtExact(file_out, 0);
    log(LogLevel::Info, "INFO: bound {} to {}\n", config.flash.deviceId,
        config.flash.driver);
#endif
    ...
    auto name = helper->readMtdFile(fmt::format("/sys/class/mtd/{}/name", mtd));
    if (name != std::string_view(partition.name))
    {
        return std::nullopt;
    }

    uint32_t size = std::stoi(
        helper->readMtdFile(fmt::format("/sys/class/mtd/{}/size", mtd)));
