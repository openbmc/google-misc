...
void BTStateMachine::calcDurations()
{
    ...
    const std::string strOSKernelDownEnd =
        std::to_string(BTTimePoint::kOSKernelDownEnd);
    const std::string strOSUserDownEndHalt =
        std::to_string(BTTimePoint::kOSUserDownEndHalt);
    const std::string strOSUserDownEndReboot =
        std::to_string(BTTimePoint::kOSUserDownEndReboot);
    const std::string strBMCDownEnd = std::to_string(BTTimePoint::kBMCDownEnd);
    const std::string strBMCStart = std::to_string(BTTimePoint::kBMCStart);
    const std::string strBIOSStart = std::to_string(BTTimePoint::kBIOSStart);
    const std::string strBIOSEnd = std::to_string(BTTimePoint::kBIOSEnd);

    if (btJson[BTCategory::kTimePoint].contains(strOSKernelDownEnd))
    {
        osKernelDownEnd =
            btJson[BTCategory::kTimePoint][strOSKernelDownEnd].get<uint64_t>();
    }
    if (btJson[BTCategory::kTimePoint].contains(strOSUserDownEndHalt))
    {
        osUserDownEndHalt = btJson[BTCategory::kTimePoint][strOSUserDownEndHalt]
                                .get<uint64_t>();
    }
    if (btJson[BTCategory::kTimePoint].contains(strOSUserDownEndReboot))
    {
        osUserDownEndReboot =
            btJson[BTCategory::kTimePoint][strOSUserDownEndReboot]
                .get<uint64_t>();
    }
    if (btJson[BTCategory::kTimePoint].contains(strBMCDownEnd))
    {
        bmcDownEnd =
            btJson[BTCategory::kTimePoint][strBMCDownEnd].get<uint64_t>();
    }
    if (btJson[BTCategory::kTimePoint].contains(strBMCStart))
    {
        bmcStart = btJson[BTCategory::kTimePoint][strBMCStart].get<uint64_t>();
    }
    if (btJson[BTCategory::kTimePoint].contains(strBIOSStart))
    {
    ...
    {
        biosEnd = btJson[BTCategory::kTimePoint][strBIOSEnd].get<uint64_t>();
    }
    ...
    if (biosStart != std::nullopt && bmcStart != std::nullopt &&
        btJson[BTCategory::kStatistic][BTStatistic::kIsACPowerCycle])
    {
    ...
    if (btJson[BTCategory::kDuration].contains(BTDuration::kTotal))
    {
        ...
        btJson[BTCategory::kDuration][BTDuration::kUnmeasured] = total;
    }
}
