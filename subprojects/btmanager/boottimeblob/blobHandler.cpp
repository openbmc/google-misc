                       ...
                       const std::string& path)
{
    ...
    statistic.set_internal_reboot_count(
        j[BTCategory::kStatistic].contains(BTStatistic::kInternalRebootCount)
            ? j[BTCategory::kStatistic][BTStatistic::kInternalRebootCount]
                  .get<int32_t>()
            : 0);
    statistic.set_power_cycle_type(
        ...
        for (size_t i = 0; i < size; i += kRowSize)
        {
            ...
            for (uint8_t j = 0; j < kCurRowSize; j++)
            {
                ss << std::setfill('0') << std::setw(2) << std::hex
                   << (static_cast<uint16_t>(vec[i + j]) & 0xff) << " ";
                if (j == 7)
                {
                    ss << ' ';
                }

                asciiStr.push_back(std::isprint(vec[i + j]) ? vec[i + j] : '.');
            }
                                       ...
                                       uint32_t requestedSize)
{
    ...
    return std::vector<uint8_t>(
        ...
        it->second.data() + std::min(offset + requestedSize,
