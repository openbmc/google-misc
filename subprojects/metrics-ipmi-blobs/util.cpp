...
bool readMem(const uint32_t target, uint32_t& memResult)
{
    ...
    void* mapBase = mmap(NULL, pageSize * 2, PROT_READ, MAP_SHARED, fd, pageOffset);
                 ...
                 double& systemdTime)
{
    ...
    powerOnCounterTime = static_cast<double>(memResult);
    kernelTime = static_cast<double>(timeMap["UserspaceTimestampMonotonic"]) / 1e6;
    systemdTime = static_cast<double>(timeMap["FinishTimestampMonotonic"]) / 1e6 - kernelTime;
